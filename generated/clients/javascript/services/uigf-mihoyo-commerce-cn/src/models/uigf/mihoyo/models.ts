...
import {
  ...
  CurrencyAndCountry,
  currencyAndCountryDeserializer,
  OrderStatus,
...
/** Common response wrapper returned by MiHoYo and HoYoLAB services. */
export interface ApiResponseCurrencyAndCountry {
  /** Vendor result code. `0` normally represents success. */
  retcode: number;
  /** Vendor diagnostic message. */
  message: string;
  /** Endpoint-specific payload. */
  data: CurrencyAndCountry;
}

export function apiResponseCurrencyAndCountryDeserializer(
  item: any,
): ApiResponseCurrencyAndCountry {
  return {
    retcode: item["retcode"],
    message: item["message"],
    data: currencyAndCountryDeserializer(item["data"]),
  };
}
