...
import {
  ...
  zenlessLedgerMonthInfoDataDeserializer,
...
/** Common response wrapper returned by MiHoYo and HoYoLAB services. */
export interface ApiResponseZenlessLedgerMonthInfoData {
  /** Vendor result code. `0` normally represents success. */
  retcode: number;
  /** Vendor diagnostic message. */
  message: string;
  /** Endpoint-specific payload. */
  data: ZenlessLedgerMonthInfoData;
}

export function apiResponseZenlessLedgerMonthInfoDataDeserializer(
  item: any,
): ApiResponseZenlessLedgerMonthInfoData {
  return {
    retcode: item["retcode"],
    message: item["message"],
    data: zenlessLedgerMonthInfoDataDeserializer(item["data"]),
  };
}
