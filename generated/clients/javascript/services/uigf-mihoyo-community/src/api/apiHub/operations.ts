...
import { MihoyoCommunityContext as Client } from "../index.js";
import {
  SignInRequest,
...
import {
  ...
  apiResponseCommunityAppConfigDataDeserializer,
  ApiResponseCommunityHomeData,
  apiResponseCommunityHomeDataDeserializer,
...
export function _getHomeSend(
  ...
  const path = expandUrlTemplate(
    "/apihub/api/home/new{?gids,cpu,device,exposed_resource_tickets,is_triggered_by_resource,parts,version}",
    {
      gids: gids,
      cpu: options?.cpu,
      device: options?.device,
      exposed_resource_tickets: options?.exposedResourceTickets,
      is_triggered_by_resource: options?.isTriggeredByResource,
      parts: options?.parts,
...
export async function _getHomeDeserialize(
  result: PathUncheckedResponse,
): Promise<ApiResponseCommunityHomeData> {
  const expectedStatuses = ["200"];
  if (!expectedStatuses.includes(result.status)) {
    throw createRestError(result);
  }

  return apiResponseCommunityHomeDataDeserializer(result.body);
}
...
export async function _getAppConfigDeserialize(
  result: PathUncheckedResponse,
): Promise<ApiResponseCommunityAppConfigData> {
  const expectedStatuses = ["200"];
  if (!expectedStatuses.includes(result.status)) {
    throw createRestError(result);
  }

  return apiResponseCommunityAppConfigDataDeserializer(result.body);
}
