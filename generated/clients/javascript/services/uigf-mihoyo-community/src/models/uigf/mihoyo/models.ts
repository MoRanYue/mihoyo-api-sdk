...
import {
  ...
  communityAppConfigDataDeserializer,
  CommunityHomeData,
  communityHomeDataDeserializer,
...
/** Common response wrapper returned by MiHoYo and HoYoLAB services. */
export interface ApiResponseCommunityAppConfigData {
  /** Vendor result code. `0` normally represents success. */
  retcode: number;
  /** Vendor diagnostic message. */
  message: string;
  /** Endpoint-specific payload. */
  data: CommunityAppConfigData;
}

export function apiResponseCommunityAppConfigDataDeserializer(
  item: any,
): ApiResponseCommunityAppConfigData {
  return {
    retcode: item["retcode"],
    message: item["message"],
    data: communityAppConfigDataDeserializer(item["data"]),
  };
}

/** Common response wrapper returned by MiHoYo and HoYoLAB services. */
export interface ApiResponseCommunityHomeData {
  /** Vendor result code. `0` normally represents success. */
  retcode: number;
  /** Vendor diagnostic message. */
  message: string;
...
export function apiResponseCommunityHomeDataDeserializer(item: any): ApiResponseCommunityHomeData {
  return {
    retcode: item["retcode"],
    message: item["message"],
    data: communityHomeDataDeserializer(item["data"]),
  };
}
