...
import { GranterLoginData, granterLoginDataDeserializer } from "./game/response/models.js";
...
/** Common response wrapper returned by MiHoYo and HoYoLAB services. */
export interface ApiResponseGranterLoginData {
  /** Vendor result code. `0` normally represents success. */
  retcode: number;
  /** Vendor diagnostic message. */
  message: string;
  /** Endpoint-specific payload. */
  data: GranterLoginData;
}

export function apiResponseGranterLoginDataDeserializer(item: any): ApiResponseGranterLoginData {
  return {
    retcode: item["retcode"],
    message: item["message"],
    data: granterLoginDataDeserializer(item["data"]),
  };
}
